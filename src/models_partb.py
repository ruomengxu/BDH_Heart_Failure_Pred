...
def logistic_regression_pred(X_train, Y_train):
	...
	Log_R=LogisticRegression(random_state=RANDOM_STATE).fit(X_train, Y_train)
	return Log_R.predict(X_train)
...
def decisionTree_pred(X_train, Y_train):
	#train a logistic regression classifier using X_train and Y_train. Use this to predict labels of X_train
	#use max_depth as 5
	DT=DecisionTreeClassifier(random_state=RANDOM_STATE,max_depth=5).fit(X_train, Y_train)
	return DT.predict(X_train)
